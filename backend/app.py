...
- 创新性：你重视个人风格，讨厌跟风和套路
- 封面：你重视封面设计的审美品位和与音乐的契合度。好的封面应该有设计感、有态度。你讨厌敷衍的自拍、滤镜堆砌、毫无设计感的默认模板。封面也是音乐表达的一部分，马虎不得。"""
...
- 封面：你认为封面是音乐审美的第一道门。好封面不一定华丽，但必须有态度、有呼吸感、跟音乐在同一个频率上。用默认模板或者随便糊弄一张照片，说明创作者对自己的作品不够尊重。"""
...
def build_review_prompt(song_name: str, artist: str, lyrics: str, audio_features: dict, critic: str, cover_url: str = "") -> str:
    """构建辣评 Prompt"""
    features_text = ""
...
"""
    ...
    cover_info = ""
    if cover_url and "default" not in cover_url.lower() and cover_url.startswith("http"):
        cover_info = f"\n封面图信息：歌曲有自定义封面图（URL: {cover_url}）。请根据封面给出审美评价——设计感、色调、与音乐风格的契合度等。"
    else:
        cover_info = "\n封面图信息：该音乐人未提供自定义封面，使用了系统默认封面。这说明创作者对视觉呈现不够重视，封面分应偏低。"

    return f"""请为以下歌曲写一段辣评，并给出六维打分（注意包含封面评分）。
...
{{
  "review": "你的辣评文字（150-250字，需包含对封面的点评）",
  "scores": {{
    "melody": 评分0-100,
    "lyrics": 评分0-100,
    "arrangement": 评分0-100,
  ...
  "total": 加权总分(melody*0.20+lyrics*0.18+arrangement*0.18+vocal*0.18+innovation*0.12+cover*0.14取整),
...
def generate_ding_review(song_info: str, lyrics: str, has_audio: bool) -> dict:
    ...
    cover = max(15, min(95, base + random.randint(-25, 10)))  # 丁大升对封面更苛刻

    total = round(melody * 0.20 + lyrics_score * 0.18 + arrangement * 0.18 + vocal * 0.18 + innovation * 0.12 + cover * 0.14)
...
def generate_liang_review(song_info: str, lyrics: str, has_audio: bool) -> dict:
    ...
    cover = max(15, min(95, base + random.randint(-20, 18)))
